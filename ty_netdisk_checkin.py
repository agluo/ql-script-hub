...
for i in range(len(ty_username)):
    print(f'开始执行帐号{i+1}:{ty_username[i]}')

    #推送函数
    def Push(contents):
        QLAPI.notify('天翼云签到',contents)
    def int2char(a):
        return BI_RM[a]
    ...
    def main():
        ...
        if (response.json()['isSign'] == "false"):
            ...
            res1 = f"{ty_username[i]}未签到，签到获得{netdiskBonus}M空间"
        else:
            print(f"已经签到过了，签到获得{netdiskBonus}M空间")
            res1 = f"{ty_username[i]}已经签到过了，签到获得{netdiskBonus}M空间"
